...
- **FR-026 (Column show/hide via header context menu)**: The ListView column header MUST respond to right-click (`NM_RCLICK` on the header subcontrol, or `WM_CONTEXTMENU` while the header has focus) by displaying a popup menu with one checkbox item per column: **Wall**, **Uptime**, **Cycle**, **Event**, **Detail**. The item is checked iff the column is currently visible. Selecting an item toggles visibility via a **logical column model**, not by zeroing column widths:

  - The dialog maintains an array of `LogicalColumn { id, headerText, defaultWidth, savedWidth, visible }` covering all five columns.
  - **Toggle** (hide or show) updates the `visible` flag on the corresponding `LogicalColumn`, then rebuilds the ListView's column set: `ListView_DeleteColumn` every column, then `ListView_InsertColumn` each `visible` `LogicalColumn` in id order, using `savedWidth` (or `defaultWidth` if never customized). This is the standard Win32 pattern Windows Explorer uses for its column-show/hide menu; it correctly cooperates with any later auto-sizing (`LVSCW_AUTOSIZE_USEHEADER`, FR-027) and avoids the zero-width-column edge cases of the alternative.
  - **User-dragged width changes** (`HDN_ENDTRACK` on the header, or `LVN_COLUMNWIDTH`-equivalent notification) MUST be captured back into the active `LogicalColumn`'s `savedWidth` so a subsequent hide/show cycle restores the user's chosen width.
...
- **FR-027 (Initial column auto-sizing)**: On dialog open, and whenever a previously-hidden column is shown for the first time in the current session, the column MUST be sized to fit its header text using `ListView_SetColumnWidth(... LVSCW_AUTOSIZE_USEHEADER)` as a one-time initial value. The result is then captured as that `LogicalColumn`'s `savedWidth` (FR-026) so subsequent show/hide cycles restore the auto-sized width rather than re-running auto-sizing.
...
- **SC-017 (Column show/hide)**: Right-clicking the column header MUST show a popup menu with five checkbox items (Wall / Uptime / Cycle / Event / Detail). Toggling any item rebuilds the ListView's column set from the logical column model (FR-026): hidden columns are completely absent from the ListView (no zero-width sliver), and shown columns appear at their prior user-customized width (or the auto-sized initial width per FR-027 for first-time-shown columns). Closing and reopening the dialog returns all columns to default visible state and re-runs auto-sizing on first show (per NFR-006).
