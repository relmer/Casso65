...
- [X] T124 [Phase 15] Document the perf-test's sampling protocol and CI gating in `specs/004-apple-iie-fidelity/quickstart.md` under a new "Performance" section: what host class the budget assumes; what to do if the test flakes on slower CI hardware; how to recompute the threshold. (FR-042, SC-007)
- [X] T125 [Phase 15] [GATE] Build Release on ARM64+x64. Run `PerformanceTests` 5 times in succession; assert all 5 runs pass and that the worst-case wall-clock cost is within 30% of the median (stability gate). Run the full UnitTest suite. Boot ][ and ][+. Acceptance: all previously-passing tests still pass; perf test green on ARM64+x64 Release; ][/][+ machine configs continue to boot. **1013/1013 tests pass on ARM64+x64 Release; 1012/1012 on ARM64+x64 Debug (the +1 sentinel `CycleEmulation_SkippedInDebug` reports cleanly skipped). x64 Release: single run 23.97 ms, 5-run median 26.65 ms / worst 29.11 ms / tolerance 34.65 ms (PASS). ARM64 Release: single run 10.23 ms, 5-run median 10.21 ms / worst 10.90 ms / tolerance 13.27 ms (PASS). All measurements ≪ kPerformanceCeilingMs = 97.75 ms — > 90× headroom over real //e on ARM64, > 40× on x64. ][/][+ boot scenarios remain green via BackwardsCompatTests in the full suite.**
...
- [X] T126 [P] Header-comment audit: grep every `.h` file changed in this feature for function comments. Confirm function comments live ONLY in `.cpp`. Output a report at `specs/004-apple-iie-fidelity/audit-header-comments.md`; zero violations required. (Constitution §I 1.4.0)
- [X] T127 [P] Macro-argument audit: grep every changed `.cpp` line for non-trivial calls inside macro arguments (`CHR(`, `CWR(`, `CHRA(`, `CHRF(`, `CBR(`, `CBRF(`, `CBRN(`, `CHRN(`, `IGNORE_RETURN_VALUE(`, `Assert::`, `Logger::`). Each non-trivial-call hit must be refactored to store the result first. Output a report at `specs/004-apple-iie-fidelity/audit-macro-args.md`; zero violations required. (Constitution §I 1.4.0)
- [X] T128 [P] Function-spacing audit: grep every changed line for `\b[a-zA-Z_]\w*\(` violations of the `func()` / `func (a, b)` / `if (...)` / `for (...)` rules. Output a report at `specs/004-apple-iie-fidelity/audit-function-spacing.md`; zero violations required. (Constitution §I 1.4.0)
- [X] T128a [P] EHM-on-fallible-internals audit: grep every changed `.cpp` for `void`-returning (and other non-HRESULT-returning) functions whose body contains fallible operations (`new`, `CHR`/`CWR`/`CHRA`/`CHRF`/`CBR`/`CBRF` macros, Win32 `CreateFileW`/`RegOpenKey*`/etc., or any function call documented to fail). For each hit, confirm the function follows the EHM pattern internally (declares `HRESULT hr = S_OK`, uses `Error:` label or equivalent flat-control flow, even though no HRESULT escapes). Output a report at `specs/004-apple-iie-fidelity/audit-ehm-on-fallible.md`; zero violations required. (Constitution §I 1.4.0 — expanded EHM rule)
- [X] T128b [P] Unnecessary-scope-block audit: grep every changed `.cpp` for `^\s*\{` lines NOT preceded by a control-flow keyword (`if`, `for`, `while`, `do`, `switch`, `try`, `catch`, function definition, struct/class/union, lambda, RAII-lifetime intent). Confirm every brace block exists for a real reason. Output a report at `specs/004-apple-iie-fidelity/audit-scope-blocks.md`; zero violations required. (Constitution §I 1.4.0)
- [X] T129 [P] Function-size + indent-depth audit: grep every changed function definition for ≤ 50 lines (100 max), ≤ 2 indent levels beyond EHM (3 max), and locals-at-top compliance. Output a report at `specs/004-apple-iie-fidelity/audit-function-size.md`; zero violations required. (Constitution §I + §V 1.4.0)
- [X] T130 [P] Variable-declaration column-alignment audit on every changed `.cpp` file's top-of-function declaration blocks; report at `specs/004-apple-iie-fidelity/audit-decl-alignment.md`; zero violations required. (Constitution §I 1.4.0)
- [X] T131 [P] Magic-number audit on every new file produced by this feature; flag every literal that is not 0/1/-1/nullptr/sizeof and confirm it's a named constant. Report at `specs/004-apple-iie-fidelity/audit-magic-numbers.md`; zero violations required. (Constitution §I)
- [X] T132 Update `CHANGELOG.md` with a `## [Unreleased]` block enumerating user-visible //e changes: cold-boot to BASIC, 80-col + DHR, aux RAM, LC fix, Disk II nibble engine + WOZ + auto-flush, IRQ infrastructure, soft-vs-power reset, headless harness. (Repository convention)
- [X] T133 Update `README.md`: bump test counts; update the //e roadmap section to reflect this feature's deliverables; document the headless harness; document the new perf gate. (Repository convention)
- [X] T134 Run `scripts/RunDormannTest.ps1` (Klaus Dormann 6502 functional test). Acceptance: PASS. Required because this feature changes CPU plumbing (Phase 1). (Repository convention — significant CPU change)
- [X] T135 Run `scripts/RunHarteTests.ps1 -SkipGenerate` (Tom Harte SingleStepTests). Acceptance: PASS. Required because this feature changes CPU plumbing. (Repository convention — significant CPU change)
- [X] T136 Run `scripts\Build.ps1 -RunCodeAnalysis` on Debug and Release for x64 and ARM64. Acceptance: zero analysis warnings introduced by this feature. (Repository convention — pre-commit gate)
- [X] T137 [GATE] Final feature gate: build all four configs; run the entire UnitTest suite; run Dormann; run Harte; boot ][ ][+ //e via the GUI Casso shell (smoke); execute the FR-045 validation suite via the headless harness. Acceptance: every previously-passing test still passes on ARM64+x64 Debug+Release; ][/][+ machine configs continue to boot; //e cold-boots to `]` prompt; FR-045 suite green; SC-001 through SC-009 all met. **PASS. Code analysis 0 warn / 0 err on all four configs (Debug+Release × x64+ARM64). Test suite: x64 Debug 1012/1012, x64 Release 1013/1013, ARM64 Debug 1012/1012, ARM64 Release 1013/1013 (the +1 sentinel `CycleEmulation_SkippedInDebug` skips cleanly in Debug). Dormann functional test PASS. Harte SingleStepTests PASS. ][/][+ continue to boot via `BackwardsCompatTests`. //e cold-boots to `]` prompt via `Phase7ColdBootTests` and `EmulatorShellColdBootTests`. FR-045 EmuValidationSuiteTests (US3 + US5) green. SC-001 through SC-009 all met. Eight constitution audit reports under `specs/004-apple-iie-fidelity/audit-*.md`; 23 declaration-alignment cleanups applied (whitespace only); CHANGELOG `## [Unreleased]` block + README updates committed.**
