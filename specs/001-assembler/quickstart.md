...
.\x64\Debug\Casso.exe assemble input.asm -o output.bin

# Assemble with listing and symbol table
.\x64\Debug\Casso.exe assemble input.asm -o output.bin -l symbols.txt -a

# Assemble and run immediately
.\x64\Debug\Casso.exe run input.asm

# Load and run a binary
.\x64\Debug\Casso.exe run program.bin --load $8000

# Run with stop address and cycle limit
.\x64\Debug\Casso.exe run input.asm --stop $FFFC --max-cycles 50000
```
...
| File | Purpose |
|---|---|
| `CassoCore/Assembler.h` | Assembler class declaration |
| `CassoCore/Assembler.cpp` | Two-pass assembler implementation |
| `CassoCore/AssemblerTypes.h` | Result structs (AssemblyResult, AssemblyError, etc.) |
| `CassoCore/Parser.h/cpp` | Line parser and operand classifier |
| `CassoCore/OpcodeTable.h/cpp` | Reverse opcode lookup (mnemonic+mode → byte) |
| `Casso/CommandLine.h/cpp` | CLI argument parsing |
| `Casso/Casso.cpp` | Main entry point with subcommand dispatch |
